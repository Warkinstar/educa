...
class PagesTests(TestCase):
    ...
    def test_course_list_view(self):
        response = self.client.get(reverse("course_list"), secure=True)
        no_response = self.client.get("no-response/", secure=True)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(no_response.status_code, 404)
        self.assertEqual(self.course.title, "Математика")

    def test_course_detail_view_for_logged_out_user(self):
        self.client.login(email="", secure=True)
        response = self.client.get(reverse("course_detail", kwargs={"slug": self.course.slug}), secure=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_course_detail_view_for_logged_in_user(self):
        self.client.login(email="testuser@example.com", password="testpass123", secure=True)
        response = self.client.get(reverse("course_detail", kwargs={"slug": self.course.slug}), secure=True)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Математика")
