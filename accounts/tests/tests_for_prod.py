"""Tests for production, to test redirect 301 code and follow to redirect"""
...
class TeacherRequestModelSignalViewTests(TestCase):
    ...
    def test_request_teacher_status_view(self):
        ...
        bad_response = self.client.post(reverse("request_teacher_status"), secure=True)
        ...
        self.client.login(email="testuser_01@email.com", password="testpass123", secure=True)

        # Get request не разрешен к этому представлению
        get_response = self.client.get(reverse("request_teacher_status"), secure=True)
        self.assertEqual(get_response.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)

        # После post запроса был выполнен редирект
        post_response = self.client.post(reverse("request_teacher_status"), secure=True)
